  ...
  return (
    <div className="relative" onClick={() => cellClickEvent(cell)}>
      ...
      {cell.duplicated && !cell.fixed && (
        <IoCloseOutline className="absolute z-10 w-12 h-12 sm:w-12 sm:h-12 text-red-700 " />
      )}
      {/* 메모 모드 */}
      <div className="flex items-center grid grid-cols-3 grid-rows-3 absolute w-12 h-12 sm:w-12 sm:h-12 text-gray-200 text-center p-0.5 cursor-pointer">
        {Array(9)
          .fill(0)
      ...
      <input
        type="text"
        className={`caret-transparent w-12 h-12 sm:w-12 sm:h-12 border font-bold border-white bg-opacity-50 dark:bg-opacity-80 dark:border-blue-100 text-center text-3xl sm:text-4xl cursor-pointer 
